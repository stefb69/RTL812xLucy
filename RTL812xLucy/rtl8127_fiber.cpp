...
#
# Ported from r8127_fiber.c of the r8127 Linux device driver released for
# Realtek 10 Gigabit Ethernet controllers with PCI-Express interface.
# Adapted to the rtl8125_private structure and helper names used by the
# merged RTL812x sources; the SerDes sequences are unchanged.
#
...
void rtl8125_clear_and_set_eth_phy_ocp_bit(struct rtl8125_private *tp, u16 addr,
...
static bool
rtl8127_wait_8127_sds_cmd_done(struct rtl8125_private *tp)
{
...
u16
rtl8127_sds_phy_read_8127(struct rtl8125_private *tp, u16 index, u16 page,
                          u16 reg)
{
...
void
rtl8127_sds_phy_write_8127(struct rtl8125_private *tp, u16 index, u16 page,
                           u16 reg, u16 val)
{
...
static void
rtl8127_clear_and_set_sds_phy_bit(struct rtl8125_private *tp, u16 index,
...
static void
rtl8127_clear_sds_phy_bit(struct rtl8125_private *tp, u16 index, u16 page,
                          u16 addr, u16 mask)
{
...
static void
rtl8127_set_sds_phy_bit(struct rtl8125_private *tp, u16 index,
...
static void
rtl8127_sds_phy_reset_8127(struct rtl8125_private *tp)
{
...
static void
rtl8127_sds_phy_reset(struct rtl8125_private *tp)
{
...
static void
rtl8127_set_sds_phy_caps_1g_8127(struct rtl8125_private *tp)
{
        u16 val;
        ...
        mdio_direct_write_phy_ocp(tp, 0xC40A, 0x0);
        mdio_direct_write_phy_ocp(tp, 0xC466, 0x0);
        mdio_direct_write_phy_ocp(tp, 0xC808, 0x0);
        mdio_direct_write_phy_ocp(tp, 0xC80A, 0x0);
...
static void
rtl8127_sds_phy_exit_1g_8127(struct rtl8125_private *tp)
{
...
static void
rtl8127_set_sds_phy_caps_10g_8127(struct rtl8125_private *tp)
{
        u16 val;
        ...
        mdio_direct_write_phy_ocp(tp, 0xC40A, 0x0);
        mdio_direct_write_phy_ocp(tp, 0xC466, 0x3);
        mdio_direct_write_phy_ocp(tp, 0xC808, 0x0);
        mdio_direct_write_phy_ocp(tp, 0xC80A, 0x0);
        rtl8125_clear_and_set_eth_phy_ocp_bit(tp,
                                              0xC804,
...
static void
rtl8127_set_sds_phy_caps_8127(struct rtl8125_private *tp)
{
...
static void
rtl8127_set_sds_phy_caps(struct rtl8125_private *tp)
{
        switch (tp->HwFiberModeVer) {
        case FIBER_MODE_RTL8127ATF:
...
static void
rtl8127_hw_sds_phy_config(struct rtl8125_private *tp)
{
        rtl8127_set_sds_phy_caps(tp);
}

void
rtl8127_hw_fiber_phy_config(struct rtl8125_private *tp)
{
        switch (tp->HwFiberModeVer) {
        case FIBER_MODE_RTL8127ATF:
...
bool
rtl8127_check_fiber_mode_support(struct rtl8125_private *tp)
{
        switch(tp->mcfg) {
        case CFG_METHOD_42: {
